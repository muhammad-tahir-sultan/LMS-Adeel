...
namespace LibraryManagementSystem
{
    public partial class AddTeacher : UserControl
    {
        ...
        {
            // Ensure that the click is within a valid row (not header)
            if (e.RowIndex >= 0)
            {
                DataGridViewRow row = addTeacher_gridView.Rows[e.RowIndex];

                // Populate fields with the selected row's data
                addTeacher_cnic.Text = row.Cells["CNIC"].Value.ToString();
                ...
                // Handle image population (if available)
                ...
                {
                    addTeacher_imagePicBox.Image = null; // Clear the image if null
                }

                // Disable editing for CNIC as it's not changeable
